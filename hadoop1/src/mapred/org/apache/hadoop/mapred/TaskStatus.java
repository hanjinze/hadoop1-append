...
public abstract class TaskStatus implements Writable, Cloneable {
  ...
  private float progress;   //任务进度，0.0-1.0
  private volatile State runState; //任务运行状态，运行状态：RUNNING, SUCCEEDED, FAILED, UNASSIGNED, KILLED, 
  									//COMMIT_PENDING, FAILED_UNCLEAN, KILLED_UNCLEAN}
  private String diagnosticInfo;  //诊断信息
  private String stateString;   //字符串形式的运行状态
  private String taskTracker;   //该tasktracker名称
  private int numSlots;  //运行该task attempt需要的slot树木，默认是1
  ...
  private volatile Phase phase = Phase.STARTING;   //任务运行阶段，STARTING, MAP, SHUFFLE, SORT, REDUCE, CLEANUP
  private Counters counters;  //该任务中定义的计数器，系统自带计数器，用户自定义计数器
  private boolean includeCounters;   //是否包含计数器，默认每隔60s发送依次
