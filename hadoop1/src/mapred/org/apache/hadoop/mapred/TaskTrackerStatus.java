...
public class TaskTrackerStatus implements Writable {
  ...
  String host;    //tasktracker 主机名
  int httpPort;   //tasktracker 对外的HTTP端口号
  int failures;   //tasktracker 上已经失败的任务总数
  List<TaskStatus> taskReports;   //当前tasktracker上各个任务运行状态
     
  volatile long lastSeen;   //上次汇报心跳的时间
  private int maxMapTasks;  //map slot总数，即允许同事运行的map task总数。由参数reduce.tasks.maximum设定
  ...
  static class ResourceStatus implements Writable {
    ...
    private long totalVirtualMemory;    //总的可用虚拟内存量
    private long totalPhysicalMemory;   //可用物理内存量
    ...
    private long availablePhysicalMemory = UNAVAILABLE; // in byte  可用的物理机内存量
    private int numProcessors = UNAVAILABLE;             //节点总的处理器个数
    private long cumulativeCpuTime = UNAVAILABLE; // in millisecond   //运行以来累计的CPU使用时间
    private long cpuFrequency = UNAVAILABLE; // in kHz   CPU 主频
    private float cpuUsage = UNAVAILABLE; // in %        CPU使用率
  ...
  private ResourceStatus resStatus;  //tasktracker资源（CPU,内存）信息
  ...
  static class TaskTrackerHealthStatus implements Writable {
    ...
    private String healthReport;  //记录导致不健康的原因
